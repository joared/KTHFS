...
class ClassifiedBoundingBoxes {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.bounding_boxes.forEach((val) => {
      bufferOffset = ClassifiedBoundingBox.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new ClassifiedBoundingBoxes(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.bounding_boxes[i] = ClassifiedBoundingBox.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.bounding_boxes.forEach((val) => {
      length += ClassifiedBoundingBox.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new ClassifiedBoundingBoxes(null);
    ...
    if (msg.bounding_boxes !== undefined) {
      ...
      for (let i = 0; i < resolved.bounding_boxes.length; ++i) {
        resolved.bounding_boxes[i] = ClassifiedBoundingBox.Resolve(msg.bounding_boxes[i]);
      }
    }
    else {
...
module.exports = ClassifiedBoundingBoxes;
