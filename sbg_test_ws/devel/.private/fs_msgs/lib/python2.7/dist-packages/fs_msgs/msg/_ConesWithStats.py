# This Python file uses the following encoding: utf-8
"""autogenerated by genpy from fs_msgs/ConesWithStats.msg. Do not edit."""
import sys
python3 = True if sys.hexversion > 0x03000000 else False
import genpy
import struct

import fs_msgs.msg
import std_msgs.msg

class ConesWithStats(genpy.Message):
  _md5sum = "6718c9a6b3d40b0468a363f35c8b1688"
  _type = "fs_msgs/ConesWithStats"
  _has_header = True #flag to mark the presence of a Header object
  _full_text = """#
# Array of cones with statistics
#

# Extra information on the message
# header.stamp: time of the message
# header.frame_id: frame in which the cones are given
Header header

# Array of cones
ConeWithStats[] cones

================================================================================
MSG: std_msgs/Header
# Standard metadata for higher-level stamped data types.
# This is generally used to communicate timestamped data 
# in a particular coordinate frame.
# 
# sequence ID: consecutively increasing ID 
uint32 seq
#Two-integer timestamp that is expressed as:
# * stamp.sec: seconds (stamp_secs) since epoch (in Python the variable is called 'secs')
# * stamp.nsec: nanoseconds since stamp_secs (in Python the variable is called 'nsecs')
# time-handling sugar is provided by the client library
time stamp
#Frame this data is associated with
string frame_id

================================================================================
MSG: fs_msgs/ConeWithStats
#
# Description of a cone with statistics
#

Cone cone
ConeStats stats

================================================================================
MSG: fs_msgs/Cone
#
# Description of a cone
#

# 2D-position of the cone
float64 x
float64 y

# Color of the cone
uint8 UNDEFINED = 0
uint8 YELLOW = 1
uint8 BLUE = 2
uint8 SMALL_ORANGE = 3
uint8 BIG_ORANGE = 4

uint8 color

# Covariance on the position [m^2] (2x2 matrix in row-major order)
float64[4] covariance

# Confidence in the detection
float64 probability

================================================================================
MSG: fs_msgs/ConeStats
#
# Statistics of a cone
#

# Number of times the cone has been seen
int32 nbr_detections

# Color counters
int32 yellow_counter
int32 blue_counter

# Average innovation (innovation = mapped position - detected position)
float32 avg_innovation_x
float32 avg_innovation_y

# Standard deviation of innovation norms
float32 std_innovation
"""
  __slots__ = ['header','cones']
  _slot_types = ['std_msgs/Header','fs_msgs/ConeWithStats[]']

  def __init__(self, *args, **kwds):
    """
    Constructor. Any message fields that are implicitly/explicitly
    set to None will be assigned a default value. The recommend
    use is keyword arguments as this is more robust to future message
    changes.  You cannot mix in-order arguments and keyword arguments.

    The available fields are:
       header,cones

    :param args: complete set of field values, in .msg order
    :param kwds: use keyword arguments corresponding to message field names
    to set specific fields.
    """
    if args or kwds:
      super(ConesWithStats, self).__init__(*args, **kwds)
      #message fields cannot be None, assign default values for those that are
      if self.header is None:
        self.header = std_msgs.msg.Header()
      if self.cones is None:
        self.cones = []
    else:
      self.header = std_msgs.msg.Header()
      self.cones = []

  def _get_types(self):
    """
    internal API method
    """
    return self._slot_types

  def serialize(self, buff):
    """
    serialize message into buffer
    :param buff: buffer, ``StringIO``
    """
    try:
      _x = self
      buff.write(_get_struct_3I().pack(_x.header.seq, _x.header.stamp.secs, _x.header.stamp.nsecs))
      _x = self.header.frame_id
      length = len(_x)
      if python3 or type(_x) == unicode:
        _x = _x.encode('utf-8')
        length = len(_x)
      buff.write(struct.pack('<I%ss'%length, length, _x))
      length = len(self.cones)
      buff.write(_struct_I.pack(length))
      for val1 in self.cones:
        _v1 = val1.cone
        _x = _v1
        buff.write(_get_struct_2dB().pack(_x.x, _x.y, _x.color))
        buff.write(_get_struct_4d().pack(*_v1.covariance))
        buff.write(_get_struct_d().pack(_v1.probability))
        _v2 = val1.stats
        _x = _v2
        buff.write(_get_struct_3i3f().pack(_x.nbr_detections, _x.yellow_counter, _x.blue_counter, _x.avg_innovation_x, _x.avg_innovation_y, _x.std_innovation))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize(self, str):
    """
    unpack serialized message in str into this message instance
    :param str: byte array of serialized message, ``str``
    """
    try:
      if self.header is None:
        self.header = std_msgs.msg.Header()
      if self.cones is None:
        self.cones = None
      end = 0
      _x = self
      start = end
      end += 12
      (_x.header.seq, _x.header.stamp.secs, _x.header.stamp.nsecs,) = _get_struct_3I().unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      end += length
      if python3:
        self.header.frame_id = str[start:end].decode('utf-8')
      else:
        self.header.frame_id = str[start:end]
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      self.cones = []
      for i in range(0, length):
        val1 = fs_msgs.msg.ConeWithStats()
        _v3 = val1.cone
        _x = _v3
        start = end
        end += 17
        (_x.x, _x.y, _x.color,) = _get_struct_2dB().unpack(str[start:end])
        start = end
        end += 32
        _v3.covariance = _get_struct_4d().unpack(str[start:end])
        start = end
        end += 8
        (_v3.probability,) = _get_struct_d().unpack(str[start:end])
        _v4 = val1.stats
        _x = _v4
        start = end
        end += 24
        (_x.nbr_detections, _x.yellow_counter, _x.blue_counter, _x.avg_innovation_x, _x.avg_innovation_y, _x.std_innovation,) = _get_struct_3i3f().unpack(str[start:end])
        self.cones.append(val1)
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill


  def serialize_numpy(self, buff, numpy):
    """
    serialize message with numpy array types into buffer
    :param buff: buffer, ``StringIO``
    :param numpy: numpy python module
    """
    try:
      _x = self
      buff.write(_get_struct_3I().pack(_x.header.seq, _x.header.stamp.secs, _x.header.stamp.nsecs))
      _x = self.header.frame_id
      length = len(_x)
      if python3 or type(_x) == unicode:
        _x = _x.encode('utf-8')
        length = len(_x)
      buff.write(struct.pack('<I%ss'%length, length, _x))
      length = len(self.cones)
      buff.write(_struct_I.pack(length))
      for val1 in self.cones:
        _v5 = val1.cone
        _x = _v5
        buff.write(_get_struct_2dB().pack(_x.x, _x.y, _x.color))
        buff.write(_v5.covariance.tostring())
        buff.write(_get_struct_d().pack(_v5.probability))
        _v6 = val1.stats
        _x = _v6
        buff.write(_get_struct_3i3f().pack(_x.nbr_detections, _x.yellow_counter, _x.blue_counter, _x.avg_innovation_x, _x.avg_innovation_y, _x.std_innovation))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize_numpy(self, str, numpy):
    """
    unpack serialized message in str into this message instance using numpy for array types
    :param str: byte array of serialized message, ``str``
    :param numpy: numpy python module
    """
    try:
      if self.header is None:
        self.header = std_msgs.msg.Header()
      if self.cones is None:
        self.cones = None
      end = 0
      _x = self
      start = end
      end += 12
      (_x.header.seq, _x.header.stamp.secs, _x.header.stamp.nsecs,) = _get_struct_3I().unpack(str[start:end])
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      start = end
      end += length
      if python3:
        self.header.frame_id = str[start:end].decode('utf-8')
      else:
        self.header.frame_id = str[start:end]
      start = end
      end += 4
      (length,) = _struct_I.unpack(str[start:end])
      self.cones = []
      for i in range(0, length):
        val1 = fs_msgs.msg.ConeWithStats()
        _v7 = val1.cone
        _x = _v7
        start = end
        end += 17
        (_x.x, _x.y, _x.color,) = _get_struct_2dB().unpack(str[start:end])
        start = end
        end += 32
        _v7.covariance = numpy.frombuffer(str[start:end], dtype=numpy.float64, count=4)
        start = end
        end += 8
        (_v7.probability,) = _get_struct_d().unpack(str[start:end])
        _v8 = val1.stats
        _x = _v8
        start = end
        end += 24
        (_x.nbr_detections, _x.yellow_counter, _x.blue_counter, _x.avg_innovation_x, _x.avg_innovation_y, _x.std_innovation,) = _get_struct_3i3f().unpack(str[start:end])
        self.cones.append(val1)
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e) #most likely buffer underfill

_struct_I = genpy.struct_I
def _get_struct_I():
    global _struct_I
    return _struct_I
_struct_d = None
def _get_struct_d():
    global _struct_d
    if _struct_d is None:
        _struct_d = struct.Struct("<d")
    return _struct_d
_struct_3I = None
def _get_struct_3I():
    global _struct_3I
    if _struct_3I is None:
        _struct_3I = struct.Struct("<3I")
    return _struct_3I
_struct_3i3f = None
def _get_struct_3i3f():
    global _struct_3i3f
    if _struct_3i3f is None:
        _struct_3i3f = struct.Struct("<3i3f")
    return _struct_3i3f
_struct_4d = None
def _get_struct_4d():
    global _struct_4d
    if _struct_4d is None:
        _struct_4d = struct.Struct("<4d")
    return _struct_4d
_struct_2dB = None
def _get_struct_2dB():
    global _struct_2dB
    if _struct_2dB is None:
        _struct_2dB = struct.Struct("<2dB")
    return _struct_2dB
